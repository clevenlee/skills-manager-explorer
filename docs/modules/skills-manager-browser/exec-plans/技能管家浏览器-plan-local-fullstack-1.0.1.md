...
| 对应技术规格 | `../skills-manager-browser-spec-1.0.md` |
| 配套 TODO | `技能管家浏览器-todo-local-fullstack-1.0.1.md` |
| 计划状态 | 待用户审核 |
| 当前门禁 | PLAN/TASKS；计划批准前不得编码 |
...
- [ ] Task 2：建立 Vue/Vite/Hono 最小可运行骨架。
- [ ] Task 3：建立配置、SQLite fixture 与结构兼容检查。
- [ ] Task 4：建立 Zod → OpenAPI → Prism 契约链。
- [ ] Task 5：实现应用状态 API。
- [ ] Task 6：实现应用壳与数据库状态体验。

#### Checkpoint A：基础可用

- [ ] `bun install --frozen-lockfile` 可复现。
- [ ] `bun run dev` 同时启动前端和本地服务，服务仅监听回环地址。
- [ ] 有效、缺失和不兼容数据库 fixture 均得到预期状态。
- [ ] OpenAPI 可生成、lint、漂移检查并启动 Prism。
- [ ] `bun run verify` 首次通过。
...
- [ ] 概览数量与 fixture SQL 结果一致。
- [ ] 来源归一化覆盖 URL、`.git`、查询参数、fallback 和未知来源。
- [ ] 来源和场景可搜索、排序、分页，并能生成可恢复的 Skill 过滤 URL；实际列表结果在 Checkpoint C 接通。
- [ ] 页面覆盖加载、空数据和无匹配结果。
- [ ] 相关 API、单元、集成和 E2E 测试通过。
...
- [ ] 名称与描述搜索、来源多选、场景多选和孤立筛选语义正确。
- [ ] 排序只接受白名单，分页边界正确。
- [ ] Skill 详情展示全部字段、空值、长文本和多个场景。
- [ ] 查询状态可通过 URL 恢复，详情返回后仍保留。
- [ ] 所有只读 PRD 验收项已有自动化证据。
...
- [ ] 来源—来源、场景—场景和来源—场景比较均正确。
- [ ] 共有、仅左、仅右和对称差数量及分页明细正确。
- [ ] 单个 Skill 归属更新具备确认、冲突检查和整体回滚。
- [ ] 数据库快照证明只修改 `scenario_skills`。
- [ ] 数据库锁定、只读、目标不存在和并发冲突均有稳定错误码。

### Phase 5：韧性、分发与交付

- [ ] Task 23：统一服务端错误与写边界防护。
- [ ] Task 24：统一前端错误、重试和窄屏体验。
...
- [ ] 核心领域与写入分支覆盖率达到 90%，整体行覆盖率达到 80%。
- [ ] 当前平台单文件可执行程序可启动并读取外部 `.env` 数据库路径。
- [ ] README、AGENTS、project-specs、OpenAPI、测试记录和交付状态一致。
- [ ] PRD 1.0 的所有验收项均有通过证据或明确未通过原因。
...
- [ ] 相关 Zod、OpenAPI、Mock、API、页面和测试保持一致。
- [ ] 没有写入真实用户数据库或扩大数据库写边界。
- [ ] 没有新增未批准依赖、端点、错误码或产品范围。
- [ ] 改动文件数与任务范围一致；超过约 5 个文件时先拆任务或记录批准理由。
- [ ] 工作区原有用户改动未被覆盖。
- [ ] TODO 已更新验证证据和剩余风险。
...
- [ ] Checkpoint A-E 全部通过。
- [ ] PRD、规格和 OpenAPI 验收项可追踪到测试。
- [ ] `bun run verify`、覆盖率和 E2E 通过。
- [ ] macOS 分发物通过干净环境 smoke。
- [ ] 未跑项和剩余风险已写入测试或 handoff 记录。

## 13. Approval Gate

本计划与配套 TODO 当前为待审核。用户明确批准计划后，方可进入 IMPLEMENT 阶段；实施时按 TODO 顺序逐项执行，不得把计划批准解释为扩大产品范围。
